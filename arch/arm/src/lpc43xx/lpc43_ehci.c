...
struct lpc43_ehci_s
{
  ...
  mutex_t excllock;             /* Support mutually exclusive access */
 ...
 *
 * Assumption:  Caller holds the excllock
 *
 ****************************************************************************/
 ...
 *
 * Assumption:  The caller holds tex EHCI excllock
 *
 ****************************************************************************/
 ...
 *
 * Assumption:  The caller does *NOT* hold the EHCI excllock.  That would
 ...
 *
 * Assumptions:  The caller holds the EHCI excllock
 *
 ****************************************************************************/
 ...
 *
 * Assumption:  The caller holds the EHCI excllock.
 *
 * Returned Value:
 ...
 *
 * Assumption:  The caller holds the EHCI excllock.  The caller must be aware
 *   that the EHCI excllock will released while waiting for the transfer to
...
static ssize_t lpc43_transfer_wait(struct lpc43_epinfo_s *epinfo)
{
   ...
   * the transfer is complete.  But we can't use the common EHCI excllock
   * or we will deadlock while waiting (because the working thread that
   * wakes this thread up needs the excllock).
   */
#warning REVISIT
  nxmutex_unlock(&g_ehci.excllock);
  ...
  ret2 = nxmutex_lock(&g_ehci.excllock);
  if (ret2 < 0)
    {
      ret = ret2;
    }
                              ...
                              uint8_t speed, uint16_t maxpacketsize)
{
  ...
  ret = nxmutex_lock(&g_ehci.excllock);
  if (ret >= 0)
    {
