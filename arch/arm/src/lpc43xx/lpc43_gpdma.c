...
{
  mutex_t excllock;        /* For exclusive access to the DMA channel list */
...
DMA_HANDLE lpc43_dmachannel(void)
{
  ...
  ret = nxmutex_lock(&g_gpdma.excllock);
  if (ret < 0)
    {
      return NULL;
    }
  ...
  nxmutex_unlock(&g_gpdma.excllock);
