...
void dma_complete(DMA_HANDLE handle, uint8_t status, void *arg)
{
  ...
  nxmutex_unlock(&dev_data->excllock);
}
...
static void update_pixels(struct ws2812_dev_s  *dev_data)
{
  ...
  /* NOTE: we don't post excllock here, the dma_complete does that */
}
...
static int my_close(struct file *filep)
{
  ...
  nxmutex_lock(&dev_data->excllock);
                        ...
                        size_t       len)
{
  ...
  /* NOTE: we don't post excllock here, so update_pixels must make sure
   *       that happens.
   */
                           ...
                           bool        has_white)
{
  ...
  nxmutex_init(&dev_data->excllock);
