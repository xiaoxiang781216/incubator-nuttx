  ...
  if (ret == OK)
    {
       ...
       * the transfer is complete.  But we can't use the common OHCI excllock
       * or we will deadlock while waiting (because the working thread that
       * wakes this thread up needs the excllock).
       */
#warning REVISIT
      nxmutex_unlock(&g_ohci.excllock);
      ...
      ret2 = nxmutex_lock(&g_ohci.excllock);
      if (ret2 < 0)
        {
          ret = ret2;
        }
                              ...
                              uint16_t maxpacketsize)
{
  ...
  ret = nxmutex_lock(&g_ohci.excllock);
  if (ret < 0)
    {
      return ret;
    }
