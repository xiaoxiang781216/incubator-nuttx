...
struct trng_dev_s
{
  mutex_t excllock;         /* Enforces exclusive access to the TRNG */
...
static ssize_t sam_read(struct file *filep, char *buffer, size_t buflen)
{
  ...
  ret = nxmutex_lock(&g_trngdev.excllock);
  if (ret < 0)
    {
...
errout:
  ...
  nxmutex_unlock(&g_trngdev.excllock);
...
static int sam_rng_initialize(void)
{
  ...
  nxmutex_init(&g_trngdev.excllock);
