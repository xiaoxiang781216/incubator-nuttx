...
struct touch_upperhalf_s
{
  ...
  mutex_t          excllock;           /* Manages exclusive access to this structure */
...
static int touch_open(FAR struct file *filep)
{
  ...
  ret = nxmutex_lock(&upper->excllock);
  if (ret < 0)
    {
...
void touch_event(FAR void *priv, FAR const struct touch_sample_s *sample)
{
  ...
  if (nxmutex_lock(&upper->excllock) < 0)
    {
      return;
    }
                   ...
                   FAR const char *path, uint8_t nums)
{
  ...
  nxmutex_init(&upper->excllock);

  ret = register_driver(path, &g_touch_fops, 0666, upper);
  if (ret < 0)
    {
      nxmutex_destroy(&upper->excllock);
      kmm_free(upper);
      return ret;
    }
