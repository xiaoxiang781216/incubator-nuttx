...
struct userled_upperhalf_s
{
  ...
  mutex_t lu_excllock;        /* Supports exclusive access to the device */
...
static int userled_open(FAR struct file *filep)
{
  ...
  ret = nxmutex_lock(&priv->lu_excllock);
  if (ret < 0)
    {
...
errout_with_lock:
  nxmutex_unlock(&priv->lu_excllock);
  return ret;
}
...
errout_with_priv:
  nxmutex_destroy(&priv->lu_excllock);
  kmm_free(priv);
  return ret;
}
