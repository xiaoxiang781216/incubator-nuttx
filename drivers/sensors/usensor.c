...
{
  mutex_t          excllock;  /* Manages exclusive access to file operations */
                            ...
                            FAR const struct sensor_reginfo_s *info)
{
  ...
  ret = nxmutex_lock(&usensor->excllock);
  if (ret < 0)
    {
  ...
  nxmutex_unlock(&usensor->excllock);
...
errout_with_lock:
  nxmutex_destroy(&usensor->excllock);
  kmm_free(usensor);
  return ret;
}
