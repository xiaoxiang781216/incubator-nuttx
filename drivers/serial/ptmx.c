...
struct ptmx_dev_s
{
  ...
  mutex_t px_excllock;              /* Supports mutual exclusion */
...
static int ptmx_open(FAR struct file *filep)
{
  ...
  ret = nxmutex_lock(&g_ptmx.px_excllock);
  if (ret < 0)
    {
      return ret;
    }
  ...
  nxmutex_unlock(&g_ptmx.px_excllock);
  return OK;
...
int ptmx_register(void)
{
  ...
  nxmutex_init(&g_ptmx.px_excllock);
...
void ptmx_minor_free(uint8_t minor)
{
  ...
  nxmutex_lock(&g_ptmx.px_excllock);
