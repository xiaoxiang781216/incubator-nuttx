...
struct pty_devpair_s
{
  ...
  mutex_t pp_excllock;          /* Mutual exclusion */
};
...
static void pty_destroy(FAR struct pty_devpair_s *devpair)
{
  ...
  nxmutex_destroy(&devpair->pp_excllock);
...
static int pty_open(FAR struct file *filep)
{
  ...
  ret = nxmutex_lock(&devpair->pp_excllock);
  if (ret < 0)
    {
      return ret;
    }
      ...
      while (devpair->pp_locked)
        {
          ...
          nxmutex_unlock(&devpair->pp_excllock);
...
int pty_register2(int minor, bool susv1)
{
  ...
  nxmutex_init(&devpair->pp_excllock);
