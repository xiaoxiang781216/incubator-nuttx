...
struct ramlog_dev_s
{
  ...
  mutex_t           rl_excllock; /* Enforces mutually exclusive access */
#ifndef CONFIG_RAMLOG_NONBLOCKING
...
#  endif
  ...
  NXMUTEX_INITIALIZER,           /* rl_excllock */
#  ifndef CONFIG_RAMLOG_NONBLOCKING
  NXSEM_INITIALIZER(0,
                             ...
                             FAR const char *buffer, size_t len)
{
  ...
  ret = nxmutex_lock(&priv->rl_excllock);
  if (ret < 0)
    {
      return ret;
    }
  ...
  nxmutex_unlock(&priv->rl_excllock);
  return len;
}
