...
struct oneshot_dev_s
{
  ...
  mutex_t od_excllock;                         /* Supports mutual exclusion */
...
static int oneshot_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
  ...
  ret = nxmutex_lock(&priv->od_excllock);
  if (ret < 0)
    {
      return ret;
    }
  ...
  nxmutex_unlock(&priv->od_excllock);
  return ret;
}
  ...
  if (ret < 0)
    {
      ...
      nxmutex_destroy(&priv->od_excllock);
      kmm_free(priv);
    }
