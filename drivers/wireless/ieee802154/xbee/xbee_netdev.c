...
struct xbeenet_driver_s
{
  ...
  mutex_t xd_excllock;              /* Exclusive access to struct */
  ...
  if (priv->xd_enableevents)
    {
      ...
      while (nxmutex_lock(&priv->xd_excllock) < 0);
      ...
      nxmutex_unlock(&priv->xd_excllock);
      return OK;
    }
                        ...
                        unsigned long arg)
{
  ...
  ret = nxmutex_lock(&priv->xd_excllock);
  if (ret < 0)
    {
