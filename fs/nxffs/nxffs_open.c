                               ...
                               FAR struct nxffs_ofile_s **ppofile)
{
  ...
  /* Get exclusive access to the volume.  Note that the volume excllock
   * protects the open file list.  Note that excllock is ALWAYS taken
   * after wrsem to avoid deadlocks.
   */

  ret = nxmutex_lock(&volume->excllock);
  if (ret < 0)
    {
   ...
   * instance.  Releasing excllock allows other readers while the write is
  ...
  *ppofile = &wrfile->ofile;
  nxmutex_unlock(&volume->excllock);
  return OK;
                                          ...
                                          FAR const char *name)
{
  ...
  /* Check every open file.  Note that the volume excllock protects the
   * list of open files.
   */
...
int nxffs_dup(FAR const struct file *oldp, FAR struct file *newp)
{
   ...
   * The volume excllock protects the open file list and, hence, would
