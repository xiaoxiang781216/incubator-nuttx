...
struct unionfs_inode_s
{
  ...
  mutex_t ui_excllock;               /* Enforces mutually exclusive access */
...
static void unionfs_destroy(FAR struct unionfs_inode_s *ui)
{
  ...
  nxmutex_destroy(&ui->ui_excllock);
  kmm_free(ui);
}
                        ...
                        int oflags, mode_t mode)
{
  ...
  ret = nxmutex_lock(&ui->ui_excllock);
  if (ret < 0)
    {
      return ret;
    }
