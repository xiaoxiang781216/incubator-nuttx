...
{
  mutex_t       excllock;       /* Enforces device exclusive access */
  ...
  if (dev)
    {
      ...
      nxmutex_init(&dev->excllock);
      nxmutex_lock(&dev->excllock);
    }
...
static int timerfd_close(FAR struct file *filep)
{
  ...
  DEBUGASSERT(nxmutex_is_locked(&priv->excllock));
                               ...
                               FAR timerfd_waiter_sem_t **slist)
{
  ...
  *slist = sem;
  ...
  if (ret < 0)
    {
       ...
       * TODO ensure that excllock wait does not fail (ECANCELED)
       */
  ...
  return nxmutex_lock(&dev->excllock);
}
...
int timerfd_create(int clockid, int flags)
{
  ...
  nxmutex_unlock(&new_dev->excllock);
