...
{
  mutex_t bs_excllock;              /* Manages exclusive access */
                                ...
                                FAR const uint8_t *adv_data, uint8_t len)
{
  ...
  ret = nxmutex_lock(&g_scanstate.bs_excllock);
  if (ret < 0)
    {
  ...
  nxmutex_unlock(&g_scanstate.bs_excllock);
}
      ...
      case SIOCBTSCANSTART:
        {
          ...
          if (g_scanstate.bs_scanning)
          ...
          else
            {
              ...
              nxmutex_init(&g_scanstate.bs_excllock);
              ...
              if (ret < 0)
                {
                  nxmutex_destroy(&g_scanstate.bs_excllock);
