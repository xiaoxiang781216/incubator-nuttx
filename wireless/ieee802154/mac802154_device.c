...
struct mac802154_chardevice_s
{
  ...
  sem_t md_excllock;                    /* Exclusive device access */
...
static int mac802154dev_open(FAR struct file *filep)
{
  ...
  ret = nxmutex_lock(&dev->md_excllock);
  if (ret < 0)
    {
...
errout_with_lock:
  nxmutex_unlock(&dev->md_excllock);
  return ret;
}
                                ...
                                FAR struct ieee802154_primitive_s *primitive)
{
  ...
  if (dev->enableevents &&
      (dev->md_open != NULL || dev->md_notify_registered))
    {
      ...
      while (nxmutex_lock(&dev->md_excllock) != 0);
...
int mac802154dev_register(MACHANDLE mac, int minor)
{
  ...
  dev->md_mac = mac;
  nxmutex_init(&dev->md_excllock); /* Allow the device to be opened once
                                    * before blocking */
...
errout_with_priv:
  nxmutex_destroy(&dev->md_excllock);
  kmm_free(dev);
  return ret;
}
