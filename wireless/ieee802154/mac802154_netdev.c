...
struct macnet_driver_s
{
  ...
  mutex_t md_excllock;            /* Exclusive access to struct */
  ...
  if (priv->md_enableevents)
    {
      ...
      while (nxmutex_lock(&priv->md_excllock) < 0);
      ...
      nxmutex_unlock(&priv->md_excllock);
      return OK;
    }
                        ...
                        unsigned long arg)
{
  ...
  ret = nxmutex_lock(&priv->md_excllock);
  if (ret < 0)
    {
